...
import { Filter, Search, LayoutGrid, Github, Check, Hourglass } from 'lucide-react';
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          {/* Filter Button */}
          <button className="w-12 h-12 flex items-center justify-center rounded-[12px] backdrop-blur-[30px] bg-white/[0.15] border border-white/25 text-[#7a6b5a] hover:bg-white/[0.2] hover:border-[#c9983a]/40 transition-all">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search"
              className="w-full pl-12 pr-4 py-3 rounded-[12px] backdrop-blur-[30px] bg-white/[0.15] border border-white/25 text-[#2d2820] placeholder-[#7a6b5a] focus:outline-none focus:bg-white/[0.2] focus:border-[#c9983a]/40 transition-all text-[13px]"
            />
          </div>
          ...
          <button 
            ...
            className="w-12 h-12 flex items-center justify-center rounded-[12px] backdrop-blur-[30px] bg-white/[0.15] border border-white/25 text-[#7a6b5a] hover:bg-white/[0.2] hover:border-[#c9983a]/40 transition-all"
          >
        ...
        {/* Table */}
        <div className="backdrop-blur-[30px] bg-white/[0.12] rounded-[20px] border border-white/20 overflow-hidden">
          <table className="w-full">
            <thead className="backdrop-blur-[20px] bg-white/[0.08] border-b border-white/20">
              <tr>
                {selectedColumns.includes('Date') && (
                  <th className={`px-6 py-4 text-left text-[12px] font-semibold uppercase tracking-wider transition-colors ${
                ...
                {selectedColumns.includes('Contributions') && (
                  <th className={`px-6 py-4 text-left text-[12px] font-semibold uppercase tracking-wider transition-colors ${
                    theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                  }`}>
                    Contributions
            ...
            <tbody>
              {rewards.map((reward, idx) => (
                ...
                >
                  {selectedColumns.includes('Date') && (
                    <td className={`px-6 py-4 text-[13px] transition-colors ${
                      theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                    }`}>{reward.date}</td>
                  )}
                  {selectedColumns.includes('ID') && (
                  ...
                  {selectedColumns.includes('Project') && (
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-base">
                          {reward.logo}
                        </div>
          ...
          <div className="fixed top-[140px] right-[40px] w-[320px] backdrop-blur-[40px] bg-white/[0.12] rounded-[16px] border border-white/30 z-50 shadow-[0_20px_60px_rgba(0,0,0,0.25)] overflow-hidden">
            ...
            <div className="px-5 pb-4 max-h-[360px] overflow-y-auto scrollbar-hide">
              <div className="space-y-2">
                {['Date', 'ID', 'Project', 'From', 'Contributions', 'Amount', 'Status']
                  ...
                  .map((column) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-5 h-5 rounded-[6px] flex items-center justify-center border-2 transition-all ${
                          isSelected 
