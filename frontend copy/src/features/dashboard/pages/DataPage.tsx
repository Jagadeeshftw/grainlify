...
export function DataPage() {
  ...
  // Base monthly data for contributor activity (used as source for all intervals)
  const baseContributorActivityData = [
  ...
  // Function to transform data based on selected interval
  const transformContributorDataByInterval = (interval: string) => {
    if (interval === 'Monthly interval') {
      return baseContributorActivityData.map(item => ({
        ...item,
        label: item.month,
        period: item.month,
      }));
    ...
    if (interval === 'Daily interval') {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        // Generate realistic daily variations based on monthly averages
        const baseValue = baseContributorActivityData[date.getMonth()];
        const variation = (Math.random() - 0.5) * 0.4; // ±20% variation
        dailyData.push({
          label: `${monthName} ${dayOfMonth}`,
          period: `${monthName} ${dayOfMonth}`,
          value: Math.round(baseValue.value * (1 + variation) / 30),
          trend: Math.round(baseValue.trend * (1 + variation) / 30),
          new: Math.round(baseValue.new * (1 + variation) / 30),
          reactivated: Math.round(baseValue.reactivated * (1 + variation) / 30),
          active: Math.round(baseValue.active * (1 + variation) / 30),
          churned: Math.round(baseValue.churned * (1 + variation) / 30),
          rewarded: Math.round(baseValue.rewarded * (1 + variation) / 30),
        });
      }
      return dailyData;
    }

    if (interval === 'Weekly interval') {
      // Generate weekly data for the last 12 weeks (approximately 3 months)
      const weeklyData = [];
      const today = new Date();
      for (let i = 11; i >= 0; i--) {
        ...
        const baseValue = baseContributorActivityData[weekStart.getMonth()];
        const variation = (Math.random() - 0.5) * 0.3; // ±15% variation
        weeklyData.push({
          label: `Week ${weekNumber}, ${monthName}`,
          period: `Week ${weekNumber}, ${monthName}`,
          value: Math.round(baseValue.value * (1 + variation) / 4),
          trend: Math.round(baseValue.trend * (1 + variation) / 4),
          new: Math.round(baseValue.new * (1 + variation) / 4),
          reactivated: Math.round(baseValue.reactivated * (1 + variation) / 4),
          active: Math.round(baseValue.active * (1 + variation) / 4),
          churned: Math.round(baseValue.churned * (1 + variation) / 4),
          rewarded: Math.round(baseValue.rewarded * (1 + variation) / 4),
        });
      }
      return weeklyData;
    }

    if (interval === 'Quarterly interval') {
      ...
      quarters.forEach(quarter => {
        const quarterMonths = baseContributorActivityData.filter((_, idx) => quarter.months.includes(idx));
        quarterlyData.push({
          label: quarter.name,
          period: `${quarter.name} 2025`,
          value: Math.round(quarterMonths.reduce((sum, m) => sum + m.value, 0) / quarterMonths.length),
          trend: Math.round(quarterMonths.reduce((sum, m) => sum + m.trend, 0) / quarterMonths.length),
          new: quarterMonths.reduce((sum, m) => sum + m.new, 0),
          reactivated: quarterMonths.reduce((sum, m) => sum + m.reactivated, 0),
          active: Math.round(quarterMonths.reduce((sum, m) => sum + m.active, 0) / quarterMonths.length),
          churned: quarterMonths.reduce((sum, m) => sum + m.churned, 0),
          rewarded: quarterMonths.reduce((sum, m) => sum + m.rewarded, 0),
        });
      });
      return quarterlyData;
    }

    if (interval === 'Yearly interval') {
      // Aggregate all monthly data into a single year
      const yearlyData = [{
        label: '2025',
        period: '2025',
        value: Math.round(baseContributorActivityData.reduce((sum, m) => sum + m.value, 0) / baseContributorActivityData.length),
        trend: Math.round(baseContributorActivityData.reduce((sum, m) => sum + m.trend, 0) / baseContributorActivityData.length),
        new: baseContributorActivityData.reduce((sum, m) => sum + m.new, 0),
        reactivated: baseContributorActivityData.reduce((sum, m) => sum + m.reactivated, 0),
        active: Math.round(baseContributorActivityData.reduce((sum, m) => sum + m.active, 0) / baseContributorActivityData.length),
        churned: baseContributorActivityData.reduce((sum, m) => sum + m.churned, 0),
        rewarded: baseContributorActivityData.reduce((sum, m) => sum + m.rewarded, 0),
      }];
      return yearlyData;
    }

    return baseContributorActivityData;
  };

  // Compute chart data based on selected interval
  const contributorActivityData = useMemo(() => {
    return transformContributorDataByInterval(contributorInterval);
  }, [contributorInterval]);
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const periodLabel = data.period || data.month || data.label || 'Period';
      return (
        <div className="backdrop-blur-[30px] bg-[#1a1410]/95 border-2 border-white/20 rounded-[12px] px-5 py-4 min-w-[200px]">
          <p className="text-[13px] font-bold text-white mb-3">{periodLabel}</p>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={contributorActivityData}>
                ...
                <XAxis
                  ...
                  stroke="#7a6b5a"
                  tick={{ fill: '#7a6b5a', fontSize: 11, fontWeight: 600 }}
                  angle={contributorInterval === 'Daily interval' || contributorInterval === 'Weekly interval' ? -45 : -45}
                  textAnchor="end"
                  height={contributorInterval === 'Daily interval' ? 100 : contributorInterval === 'Weekly interval' ? 90 : 80}
                  interval={contributorInterval === 'Daily interval' ? 2 : contributorInterval === 'Weekly interval' ? 1 : 0}
                />
                <YAxis stroke="#7a6b5a" tick={{ fill: '#7a6b5a', fontSize: 11, fontWeight: 600 }} />
                <Tooltip content={<CustomTooltip />} />
                <Bar
