...
export function EcosystemDetailPage({ ecosystemId, ecosystemName, initialDescription, initialLogoUrl, onBack, onProjectClick }: EcosystemDetailPageProps) {
  ...
  const normalizeKeyAreas = (raw: unknown): Array<{ title: string; description: string }> => {
    ...
    return raw
      .map((k) => ({
        ...
        description:
          k && typeof k === "object" && "description" in k && typeof (k as { description?: unknown }).description === "string"
  ...
  const displayDescription =
    apiDescription ||
    (initialDescription != null && initialDescription !== "" ? String(initialDescription).trim() : "") ||
    (hasDetail ? "" : "Projects building decentralized protocols, tooling, and infrastructure.");
  ...
  const ecosystemData = {
    ...
    keyAreas:
      hasDetail && apiKeyAreas.length > 0
        ? apiKeyAreas
        : hasDetail
          ? []
          : [
    ...
    technologies:
      hasDetail && apiTechnologies.length > 0
        ? apiTechnologies
        : hasDetail
          ? []
          : [
  ...
  const filteredProjects = ecosystemProjects.filter(
    (project) => project.name.toLowerCase().includes(searchQuery.toLowerCase()) || project.description.toLowerCase().includes(searchQuery.toLowerCase()),
  );
    ...
    <div className="h-full overflow-y-auto px-4 md:px-0">
      ...
      <div className="mb-4 md:mb-6 flex items-center gap-1.5 md:gap-2 ml-0 md:ml-12 overflow-x-auto scrollbar-hide">
        <button
          ...
          className={`text-[12px] md:text-[14px] font-semibold transition-colors touch-manipulation whitespace-nowrap flex-shrink-0 ${
            isDark ? "text-[#d4d4d4] hover:text-[#c9983a] active:text-[#c9983a]" : "text-[#7a6b5a] hover:text-[#a67c2a] active:text-[#a67c2a]"
          }`}
        >
          Ecosystems
        </button>
        <ChevronRight className={`w-3 h-3 md:w-4 md:h-4 flex-shrink-0 ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`} />
        <span className={`text-[12px] md:text-[14px] font-bold transition-colors whitespace-nowrap flex-shrink-0 ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
          {ecosystemName}
        </span>
        <ChevronRight className={`w-3 h-3 md:w-4 md:h-4 flex-shrink-0 ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`} />
        <span className={`text-[12px] md:text-[14px] font-semibold transition-colors whitespace-nowrap flex-shrink-0 ${isDark ? "text-[#c9983a]" : "text-[#a67c2a]"}`}>
          Overview
        </span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
        ...
        <div className="flex-[1] flex-shrink-0 space-y-4 md:space-y-6">
          ...
          <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
            <div className="flex items-center gap-3 md:gap-4 mb-3 md:mb-4">
              ...
              <div className="flex-1 min-w-0">
                <h1 className={`text-[16px] md:text-[20px] font-bold transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
          ...
          {/* Description */}
          <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
            <h2 className={`text-[14px] md:text-[16px] font-bold mb-2 md:mb-3 transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>Description</h2>
            <p className={`text-[12px] md:text-[13px] leading-relaxed transition-colors ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>{ecosystemData.description}</p>
          </div>

          {/* Languages - only show when configured (optional for future) */}
          {ecosystemData.languages.length > 0 && (
            <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
              <h2 className={`text-[14px] md:text-[16px] font-bold mb-2 md:mb-3 transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>Languages</h2>
              <div className="flex flex-wrap gap-2 md:gap-3">
                {ecosystemData.languages.map((lang, idx) => (
                  <div key={idx} className="px-2.5 md:px-3 py-1 md:py-1.5 rounded-[6px] md:rounded-[8px] backdrop-blur-[20px] border border-white/25 bg-white/[0.08]">
                    <span className="text-[11px] md:text-[12px] font-semibold text-[#c9983a]">{lang.name}</span>
                    <span className={`ml-1.5 md:ml-2 text-[10px] md:text-[11px] ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>{lang.percentage}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Links */}
          <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
            <h2 className={`text-[14px] md:text-[16px] font-bold mb-3 md:mb-4 transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>Links</h2>
            <div className="space-y-2 md:space-y-3">
              {ecosystemData.links.map((link, idx) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className={`text-[12px] md:text-[13px] font-semibold transition-colors truncate ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
                      {link.label}
                    </div>
                    <div className={`text-[10px] md:text-[11px] transition-colors truncate ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>{link.url}</div>
                  </div>
        ...
        <div className="flex-[3] min-w-0">
          ...
          {activeTab === "overview" && (
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                <div className="backdrop-blur-[40px] rounded-[12px] md:rounded-[16px] border bg-white/[0.12] border-white/20 p-3 md:p-5">
                  <div className="flex flex-col md:flex-row md:items-center gap-1.5 md:gap-2 mb-1.5 md:mb-2">
                    ...
                    <span className={`text-[9px] md:text-[11px] font-bold uppercase tracking-wide leading-tight ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
                      Active Contributors
                    </span>
                  </div>
              ...
              {/* About Section */}
              <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
                <h2 className={`text-[16px] md:text-[18px] font-bold mb-3 md:mb-4 transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
                  About {ecosystemName}
                </h2>
                <p className={`text-[12px] md:text-[14px] leading-relaxed transition-colors ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>{ecosystemData.about}</p>
              </div>

              {/* Key Areas */}
              <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
                <h2 className={`text-[16px] md:text-[18px] font-bold mb-3 md:mb-4 transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>Key Areas</h2>
                <ul className="space-y-2 md:space-y-3">
                  {ecosystemData.keyAreas.map((area, idx) => (
                    <li key={idx} className="flex gap-2 md:gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`font-bold text-[12px] md:text-[14px] ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>{area.title}:</span>{" "}
                        <span className={`text-[12px] md:text-[14px] ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>{area.description}</span>
                      </div>
                    </li>
                  ))}
                </ul>
              ...
              {/* Technologies */}
              <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-4 md:p-6">
                <h2 className={`text-[16px] md:text-[18px] font-bold mb-3 md:mb-4 transition-colors ${isDark ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>Technologies</h2>
                ...
                <ul className="space-y-1.5 md:space-y-2">
                  {ecosystemData.technologies.map((tech, idx) => (
                    <li key={idx} className="flex gap-2 md:gap-3">
                      <span className={`mt-0.5 md:mt-1 flex-shrink-0 ${isDark ? "text-[#c9983a]" : "text-[#a67c2a]"}`}>•</span>
                      <span className={`text-[12px] md:text-[14px] ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>{tech}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {activeTab === "projects" && (
            <div className="space-y-4 md:space-y-6">
              ...
              {isLoadingProjects ? (
                ...
                <EmptyState
                  variant="no-ecosystems"
                  isDark={isDark}
                  headline={`No projects in ${ecosystemName} yet`}
                  subtext="Projects added under this ecosystem will appear here."
                />
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
          ...
          {activeTab === "community" && (
            <div className="backdrop-blur-[40px] rounded-[16px] md:rounded-[24px] border bg-white/[0.12] border-white/20 p-6 md:p-8 text-center">
              <Users className={`w-10 h-10 md:w-12 md:h-12 mx-auto mb-3 md:mb-4 ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`} />
              <p className={`text-[12px] md:text-[14px] ${isDark ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>Community view coming soon</p>
            </div>
          )}
        </div>
      </div>
