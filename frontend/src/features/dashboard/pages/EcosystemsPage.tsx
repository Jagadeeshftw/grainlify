...
export function EcosystemsPage({ onEcosystemClick }: EcosystemsPageProps) {
  ...
  const filteredEcosystems = ecosystems.filter(
    (eco) => eco.name.toLowerCase().includes(searchQuery.toLowerCase()) || (eco.description && eco.description.toLowerCase().includes(searchQuery.toLowerCase())),
  );
    ...
    <div className="space-y-4 md:space-y-6 px-4 md:px-0">
      ...
      <div
        className={`backdrop-blur-[40px] bg-gradient-to-br rounded-[20px] md:rounded-[28px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-6 md:p-10 transition-colors ${
          theme === "dark" ? "from-white/[0.08] to-white/[0.04] border-white/10" : "from-white/[0.15] to-white/[0.08] border-white/20"
        }`}
      >
        <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
          <div className="flex-1">
            <h1 className={`text-[24px] md:text-[36px] font-bold mb-2 md:mb-3 transition-colors ${theme === "dark" ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
              Explore Ecosystems
            </h1>
            <p className={`text-[14px] md:text-[16px] max-w-3xl transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
          ...
          {Array.from({ length: 6 }).map((_, idx) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4 md:mb-5">
                <div className={`w-12 h-12 md:w-14 md:h-14 rounded-[12px] md:rounded-[14px] ${theme === "dark" ? "bg-white/10" : "bg-black/10"}`} />
              </div>
              ...
              <div className="flex items-center gap-6 mt-4 mb-4">
                <div className="flex-1">
                  <div className={`h-3 w-16 rounded ${theme === "dark" ? "bg-white/10" : "bg-black/10"}`} />
                  <div className={`h-6 w-10 rounded mt-2 ${theme === "dark" ? "bg-white/10" : "bg-black/10"}`} />
                </div>
                <div className="flex-1">
              ...
              {/* Description */}
              <div className={`h-3 w-full rounded ${theme === "dark" ? "bg-white/10" : "bg-black/10"}`} />
              <div className={`h-3 w-5/6 rounded mt-2 ${theme === "dark" ? "bg-white/10" : "bg-black/10"}`} />

              {/* Footer */}
              <div className="mt-4">
                <div className={`h-4 w-28 rounded ${theme === "dark" ? "bg-white/10" : "bg-black/10"}`} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`text-center py-8 md:py-12 px-4 ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
          <p className={`text-[14px] md:text-[16px] ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
            {searchQuery ? (
              ...
              <EmptyState variant="no-ecosystems" isDark={theme === "dark"} ctaLabel="Request Ecosystem" onCta={() => setShowRequestModal(true)} />
            )}
          </p>
          {!isLoading && ecosystems.length > 0 && <div className="mt-2 text-[11px] md:text-xs opacity-70">(Filtered from {ecosystems.length} ecosystems)</div>}
        </div>
      ) : (
          ...
          {filteredEcosystems.map((ecosystem) => {
            ...
            return (
              ...
              >
                ...
                {/* Title */}
                <h3 className={`text-[16px] md:text-[18px] font-bold mb-2 transition-colors ${theme === "dark" ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
                  {ecosystem.name}
                </h3>
                ...
                <div className="flex items-center gap-4 md:gap-6 mb-3 md:mb-4">
                  <div>
                    <div className={`text-[10px] md:text-[11px] mb-0.5 transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>Projects</div>
                    <div className={`text-[18px] md:text-[20px] font-bold transition-colors ${theme === "dark" ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
                      {ecosystem.projects}
                    </div>
                  </div>
                  <div>
                    <div className={`text-[10px] md:text-[11px] mb-0.5 transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>Contributors</div>
                    <div className={`text-[18px] md:text-[20px] font-bold transition-colors ${theme === "dark" ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
                ...
                {/* Description */}
                <p className={`text-[12px] md:text-[13px] mb-4 md:mb-5 leading-relaxed transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
        ...
        >
          ...
          <div className="relative z-10">
            <div className="text-center max-w-2xl mx-auto">
              ...
              <h3 className={`text-[22px] md:text-[28px] font-bold mb-3 md:mb-4 transition-colors ${theme === "dark" ? "text-[#f5f5f5]" : "text-[#2d2820]"}`}>
                Missing Your Ecosystem?
              </h3>

              <p className={`text-[14px] md:text-[16px] mb-5 md:mb-6 leading-relaxed transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Add New Ecosystem" width="md">
      ...
      <Modal
        ...
        maxHeight
      >
        <p className={`text-[13px] md:text-[14px] mb-4 md:mb-6 transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}>
        ...
        <form onSubmit={handleRequestSubmit}>
          ...
          <ModalFooter>
            <ModalButton onClick={() => setShowRequestModal(false)}>Cancel</ModalButton>
            <ModalButton type="submit" variant="primary">
