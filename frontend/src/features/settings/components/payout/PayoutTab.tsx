...
export function PayoutTab() {
  ...
  return (
    <div className="space-y-6">
      {errorMessage && <div className="bg-red-500/10 border border-red-500/50 text-red-500 p-3 rounded-lg mb-4">{errorMessage}</div>}
      ...
      >
        {isLoading ? (
          ...
          <>
            <h2 className={`text-[28px] font-bold mb-2 transition-colors ${theme === "dark" ? "text-[#f5efe5]" : "text-[#2d2820]"}`}>Payout preferences</h2>
            <p className={`text-[14px] mb-8 transition-colors ${theme === "dark" ? "text-[#b8a898]" : "text-[#7a6b5a]"}`}>
        ...
        {isLoading ? (
          ...
          <EmptyState variant="no-payout-history" isDark={theme === "dark"} ctaLabel="Browse projects" onCta={() => {}} />
        ) : (
          <>
            {/* Project List */}
            <div className="space-y-4 mb-6">
              {/* Header Row */}
              <div className="grid grid-cols-2 gap-4 pb-3 border-b border-white/10">
                <div className={`text-[13px] font-semibold transition-colors ${theme === "dark" ? "text-[#f5efe5]" : "text-[#2d2820]"}`}>Project</div>
                <div className={`text-[13px] font-semibold transition-colors ${theme === "dark" ? "text-[#f5efe5]" : "text-[#2d2820]"}`}>Billing profile</div>
              </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {project.language ? (
                        ...
                        <span className={`text-[14px] font-bold transition-colors ${theme === "dark" ? "text-[#d4c5b0]" : "text-[#2d2820]"}`}>
                    ...
                    <div className="flex flex-col">
                      ...
                      {project.ecosystem_name && (
                        <span className={`text-[12px] transition-colors ${theme === "dark" ? "text-[#8a7e70]" : "text-[#7a6b5a]"}`}>{project.ecosystem_name}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center">
                    {availableProfiles.length === 0 ? (
                      <div className={`px-4 py-2.5 rounded-[12px] text-[14px] transition-colors ${theme === "dark" ? "text-[#8a7e70]" : "text-[#7a6b5a]"}`}>
                      ...
                      <select
                        ...
                        className={`w-full max-w-[300px] px-4 py-2.5 rounded-[12px] backdrop-blur-[30px] border text-[14px] focus:outline-none focus:bg-white/[0.2] focus:border-[#c9983a]/30 transition-all ${
                          theme === "dark" ? "bg-[#3d342c]/[0.4] border-white/15 text-[#f5efe5]" : "bg-white/[0.15] border-white/25 text-[#2d2820]"
                        }`}
                      >
            ...
            <div
              className={`flex items-start gap-2 p-4 rounded-[14px] backdrop-blur-[30px] border transition-colors ${
                theme === "dark" ? "bg-[#3d342c]/[0.3] border-white/10" : "bg-white/[0.08] border-white/15"
              }`}
            >
              <Info className={`w-5 h-5 flex-shrink-0 mt-0.5 transition-colors ${theme === "dark" ? "text-[#8a7e70]" : "text-[#7a6b5a]"}`} />
              <p className={`text-[13px] transition-colors ${theme === "dark" ? "text-[#b8a898]" : "text-[#7a6b5a]"}`}>
                {availableProfiles.length === 0
