...
fn test_rbac_emergency_withdraw_drains_all_funds() {
    ...
    // Mint and distribute funds to program
    let depositor = Address::generate(&env);
...
fn test_rbac_after_emergency_withdraw_can_unpause_and_reuse() {
    ...
    // After locking, program data should reflect new funds even though token balance remains 0
    assert_eq!(contract_client.get_remaining_balance(), 200);
}
