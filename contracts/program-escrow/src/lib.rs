...
pub enum DataKey {
    ...
    Paused(String),                  // program_id -> bool (paused state)
    PauseReason(String),             // program_id -> String (pause reason)
    PausedAt(String),                // program_id -> u64 (pause timestamp)
}
...
impl ProgramEscrowContract {
    ...
    pub fn lock_program_funds(env: Env, program_id: String, amount: i128) -> ProgramData {
        ...
        // Check pause state for this program
        if env
            .storage()
            .instance()
            .get::<DataKey, bool>(&DataKey::Paused(program_id.clone()))
            .unwrap_or(false)
        {
    ...
    /// Pauses a program, blocking lock_program_funds, batch_payout, and single_payout.
    ///
    /// Only the `authorized_payout_key` of the program may call this.
    ///
    /// # Events
    /// Emits: `("prog", "paused")` with `(program_id, authorized_payout_key, reason, timestamp)`
    pub fn pause_program(env: Env, program_id: String, reason: soroban_sdk::String) {
        let program_key = DataKey::Program(program_id.clone());
        let program_data: ProgramData = env
            .storage()
            .instance()
        ...
        env.storage()
            .instance()
            .set(&DataKey::Paused(program_id.clone()), &true);
        env.storage()
            .instance()
            .set(&DataKey::PauseReason(program_id.clone()), &reason);
        env.storage()
            .instance()
            .set(&DataKey::PausedAt(program_id.clone()), &now);

        env.events().publish(
            (symbol_short!("prog"), symbol_short!("paused")),
            (program_id, program_data.authorized_payout_key, reason, now),
        );
    }

    /// Unpauses a program, resuming normal operations.
    ///
    /// Only the `authorized_payout_key` of the program may call this.
    ///
    /// # Events
    /// Emits: `("prog", "unpaused")` with `(program_id, authorized_payout_key, timestamp)`
    pub fn unpause_program(env: Env, program_id: String) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::Paused(program_id.clone()), &false);
        env.storage()
            .instance()
            .remove(&DataKey::PauseReason(program_id.clone()));
        env.storage()
            .instance()
            .remove(&DataKey::PausedAt(program_id.clone()));

        env.events().publish(
            (symbol_short!("prog"), symbol_short!("unpaused")),
            (
                program_id,
                program_data.authorized_payout_key,
    ...
    /// Returns whether a program is currently paused.
    pub fn is_program_paused(env: Env, program_id: String) -> bool {
        env.storage()
            .instance()
            .get::<DataKey, bool>(&DataKey::Paused(program_id))
            .unwrap_or(false)
    }

    /// Returns the pause reason for a program, if any.
    pub fn get_program_pause_reason(env: Env, program_id: String) -> Option<soroban_sdk::String> {
        env.storage()
            .instance()
            .get(&DataKey::PauseReason(program_id))
    }

    /// Emergency withdrawal: transfers the entire remaining balance of a program
    /// to a designated recipient. Only the `authorized_payout_key` may call this.
    ///
    /// # Events
    /// Emits: `("prog", "emrg_wd")` with `(program_id, authorized_payout_key, recipient, amount, timestamp)`
    pub fn emergency_withdraw(env: Env, program_id: String, recipient: Address) {
        let program_key = DataKey::Program(program_id.clone());
        let mut program_data: ProgramData = env
            .storage()
            .instance()
        ...
        env.events().publish(
            (symbol_short!("prog"), symbol_short!("emrg_wd")),
            (
                program_id,
                program_data.authorized_payout_key,
                recipient,
...
mod test {
    ...
    fn setup_test_program_with_real_token(
        env: &Env,
    ) -> (
        ...
        let program_id = String::from_str(env, "pause-test-program");
        let token_contract = env.register_stellar_asset_contract_v2(auth_key.clone());
        let token_addr = token_contract.address();
        let token_admin = soroban_sdk::token::StellarAssetClient::new(env, &token_addr);
        let token_client = soroban_sdk::token::Client::new(env, &token_addr);
        env.mock_all_auths();
        client.initialize_program(&program_id, &auth_key, &token_addr);
        let amount = 1_000_000_000_0000000i128;
        token_admin.mint(&auth_key, &amount);
        token_client.transfer(&auth_key, &contract_id, &amount);
        client.lock_program_funds(&program_id, &amount);
        (client, contract_id, auth_key, token_client, program_id)
    }

    // --- pause_program ---
    #[test]
    fn test_pause_program_by_authorized_key_succeeds() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        ...
        assert_eq!(client.get_program_pause_reason(&program_id), Some(reason));
    }

    #[test]
    #[should_panic]
    fn test_pause_program_by_non_authorized_key_fails() {
        let env = Env::default();
        let contract_id = env.register_contract(None, ProgramEscrowContract);
        let client = ProgramEscrowContractClient::new(&env, &contract_id);
        let auth_key = Address::generate(&env);
        ...
        env.mock_all_auths();
        client.initialize_program(&program_id, &auth_key, &token);
        let non_auth = Address::generate(&env);
        non_auth.require_auth();
        client.pause_program(&program_id, &String::from_str(&env, "hack"));
    }

    // --- unpause_program ---
    #[test]
    fn test_unpause_program_by_authorized_key_succeeds() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "test"));
        assert!(client.is_program_paused(&program_id));
        client.unpause_program(&program_id);
        assert!(!client.is_program_paused(&program_id));
        assert_eq!(client.get_program_pause_reason(&program_id), None);
    }

    #[test]
    #[should_panic]
    fn test_unpause_program_by_non_authorized_key_fails() {
        let env = Env::default();
        let contract_id = env.register_contract(None, ProgramEscrowContract);
        let client = ProgramEscrowContractClient::new(&env, &contract_id);
        let auth_key = Address::generate(&env);
    ...
    // --- single_payout blocked while paused ---
    #[test]
    #[should_panic(expected = "Contract is paused")]
    fn test_single_payout_while_paused_fails() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "security"));
        let recipient = Address::generate(&env);
        client.single_payout(&program_id, &recipient, &100_0000000i128);
    }

    #[test]
    fn test_single_payout_while_paused_no_state_change() {
        let env = Env::default();
        let (client, _, _, token, program_id) = setup_test_program_with_real_token(&env);
        let balance_before = client.get_remaining_balance(&program_id);
        client.pause_program(&program_id, &String::from_str(&env, "security"));
        let recipient = Address::generate(&env);
        let result = client.try_single_payout(&program_id, &recipient, &100_0000000i128);
        assert!(result.is_err());
        assert_eq!(client.get_remaining_balance(&program_id), balance_before);
        assert_eq!(token.balance(&recipient), 0);
    }

    // --- batch_payout blocked while paused ---
    #[test]
    #[should_panic(expected = "Contract is paused")]
    fn test_batch_payout_while_paused_fails() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "security"));
        let recipient = Address::generate(&env);
        let recipients = soroban_sdk::vec![&env, recipient];
        let amounts = soroban_sdk::vec![&env, 100_0000000i128];
        client.batch_payout(&program_id, &recipients, &amounts);
    }

    // --- lock_program_funds blocked while paused ---
    #[test]
    #[should_panic(expected = "Contract is paused")]
    fn test_lock_program_funds_while_paused_fails() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "security"));
        client.lock_program_funds(&program_id, &500_0000000i128);
    }

    // --- emergency_withdraw ---
    #[test]
    fn test_emergency_withdraw_drains_program_balance() {
        let env = Env::default();
        let (client, _, _, token, program_id) = setup_test_program_with_real_token(&env);
        let balance_before = client.get_remaining_balance(&program_id);
        assert!(balance_before > 0);
        client.pause_program(&program_id, &String::from_str(&env, "emergency"));
        let recipient = Address::generate(&env);
        client.emergency_withdraw(&program_id, &recipient);
        assert_eq!(client.get_remaining_balance(&program_id), 0);
        assert_eq!(token.balance(&recipient), balance_before);
    }

    #[test]
    #[should_panic]
    fn test_emergency_withdraw_by_non_authorized_key_fails() {
        let env = Env::default();
        let contract_id = env.register_contract(None, ProgramEscrowContract);
        let client = ProgramEscrowContractClient::new(&env, &contract_id);
        let auth_key = Address::generate(&env);
    ...
    #[test]
    fn test_program_operations_resume_after_unpause() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "temporary"));
        assert!(client.is_program_paused(&program_id));
        client.unpause_program(&program_id);
        assert!(!client.is_program_paused(&program_id));
        let recipient = Address::generate(&env);
    ...
    // --- event emission ---
    #[test]
    fn test_pause_program_emits_event() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        ...
        assert!(
            ...
            "pause_program() must emit an event"
        );
    }

    #[test]
    fn test_unpause_program_emits_event() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "test"));
        let before = env.events().all().len();
        ...
        assert!(
            env.events().all().len() > before,
            "unpause_program() must emit an event"
        );
    }

    #[test]
    fn test_program_emergency_withdraw_emits_event() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        client.pause_program(&program_id, &String::from_str(&env, "emergency"));
        let before = env.events().all().len();
        ...
        assert!(
            env.events().all().len() > before,
            "emergency_withdraw() must emit an event"
        );
    }

    // --- edge cases ---
    #[test]
    fn test_pause_program_when_already_paused_updates_reason() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        let r1 = String::from_str(&env, "reason one");
        let r2 = String::from_str(&env, "updated reason");
        client.pause_program(&program_id, &r1);
        assert_eq!(client.get_program_pause_reason(&program_id), Some(r1));
        client.pause_program(&program_id, &r2);
        assert!(client.is_program_paused(&program_id));
        assert_eq!(client.get_program_pause_reason(&program_id), Some(r2));
    }

    #[test]
    fn test_unpause_program_when_not_paused_is_safe() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        assert!(!client.is_program_paused(&program_id));
        client.unpause_program(&program_id); // must not panic
        assert!(!client.is_program_paused(&program_id));
    }

    #[test]
    fn test_program_emergency_withdraw_zero_balance_is_safe() {
        let env = Env::default();
        let contract_id = env.register_contract(None, ProgramEscrowContract);
        let client = ProgramEscrowContractClient::new(&env, &contract_id);
        let auth_key = Address::generate(&env);
        let token = Address::generate(&env);
        let program_id = String::from_str(&env, "zero-bal");
        env.mock_all_auths();
        client.initialize_program(&program_id, &auth_key, &token);
        let recipient = Address::generate(&env);
        client.emergency_withdraw(&program_id, &recipient); // must not panic
        assert_eq!(client.get_remaining_balance(&program_id), 0);
    }

    #[test]
    fn test_program_multiple_pause_unpause_cycles() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        for _ in 0..3 {
            client.pause_program(&program_id, &String::from_str(&env, "cycle"));
            assert!(client.is_program_paused(&program_id));
            let result =
                client.try_single_payout(&program_id, &Address::generate(&env), &100_0000000i128);
            assert!(result.is_err());
            client.unpause_program(&program_id);
            assert!(!client.is_program_paused(&program_id));
        }
    ...
    #[test]
    fn test_program_emergency_withdraw_works_without_prior_pause() {
        let env = Env::default();
        let (client, _, _, token, program_id) = setup_test_program_with_real_token(&env);
        let bal = client.get_remaining_balance(&program_id);
        assert!(bal > 0);
        assert!(!client.is_program_paused(&program_id));
        let recipient = Address::generate(&env);
        client.emergency_withdraw(&program_id, &recipient);
        assert_eq!(client.get_remaining_balance(&program_id), 0);
        assert_eq!(token.balance(&recipient), bal);
    }

    #[test]
    fn test_program_pause_reason_cleared_after_unpause() {
        let env = Env::default();
        let (client, _, _, _, program_id) = setup_test_program_with_real_token(&env);
        assert_eq!(client.get_program_pause_reason(&program_id), None);
        let reason = String::from_str(&env, "regulatory");
        client.pause_program(&program_id, &reason);
        assert_eq!(client.get_program_pause_reason(&program_id), Some(reason));
        client.unpause_program(&program_id);
        assert!(!client.is_program_paused(&program_id));
