...
pub struct ThresholdConfig {
    ...
    /// Minimum cooldown period before reopening
    pub cooldown_period_secs: Duration,
...
pub fn init_threshold_monitor(env: &Env) {
    ...
    let metrics = WindowMetrics::new(grainlify_time::now(env));
    env.storage()
        .persistent()
...
pub fn get_current_metrics(env: &Env) -> WindowMetrics {
    env.storage()
        ...
        .unwrap_or_else(|| WindowMetrics::new(grainlify_time::now(env)))
}
...
fn rotate_window_if_needed(env: &Env) {
    ...
    let window_end = metrics.window_start.add_duration(config.time_window_secs);
...
pub fn is_cooldown_active(env: &Env) -> bool {
    let last_cooldown_end: Timestamp = env
        .storage()
        .persistent()
...
pub fn apply_cooldown(env: &Env) {
    ...
    let cooldown_duration = config.cooldown_period_secs.saturating_mul(multiplier as u64);
    let cooldown_end = now.add_duration(cooldown_duration);
...
fn emit_metrics_reset_event(env: &Env, admin: &Address, timestamp: Timestamp) {
    env.events().publish(
