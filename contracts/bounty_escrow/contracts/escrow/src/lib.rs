...
pub enum Error {
    ...
    /// Returned when an operation is attempted while the contract is paused
    ContractPaused = 18,
}
...
pub enum DataKey {
    ...
    PausedAt,             // u64 - timestamp when paused
}
...
impl BountyEscrowContract {
    ...
    /// # Arguments
    /// * `env` - The contract environment
    /// * `reason` - Human-readable pause reason for off-chain monitoring
    ///
    /// # Returns
    ...
    ///
    /// # Events
    /// Emits: `("escrow", "paused")` with `(admin, reason, timestamp)`
    pub fn pause(env: Env, reason: soroban_sdk::String) -> Result<(), Error> {
        if !env.storage().instance().has(&DataKey::Admin) {
            return Err(Error::NotInitialized);
        }
        let admin: Address = env.storage().instance().get(&DataKey::Admin).unwrap();
    ...
    ///
    /// # Events
    /// Emits: `("escrow", "unpaused")` with `(admin, timestamp)`
    ...
    /// Emergency withdrawal: transfers the entire contract token balance to a
    /// designated recipient. Admin-only. Typically called while paused.
    ///
    /// # Arguments
    /// * `env` - The contract environment
    /// * `recipient` - Address to receive all contract funds
    ///
    /// # Returns
    /// * `Ok(())` - All funds transferred successfully
    /// * `Err(Error::NotInitialized)` - Contract not initialized
    /// * `Err(Error::Unauthorized)` - Caller is not the admin
    ///
    /// # Events
    /// Emits: `("escrow", "emrg_wd")` with `(admin, recipient, amount, timestamp)`
    pub fn emergency_withdraw(env: Env, recipient: Address) -> Result<(), Error> {
        ...
        if balance > 0 {
            client.transfer(&contract_address, &recipient, &balance);
        }

        env.events().publish(
            (symbol_short!("escrow"), symbol_short!("emrg_wd")),
            (admin, recipient, balance, env.ledger().timestamp()),
        );
